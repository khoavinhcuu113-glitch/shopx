...
// Trạng thái đơn dịch vụ — dùng câu chữ chung, không đoán theo tên nghề
const STATUS = {
  waiting:     { key: 'waiting',     label: '⏳ Chờ đối tác bắt đầu',        color: '#f59e0b', bg: '#fff8e1' },
  working:     { key: 'working',     label: '🔨 Đang thực hiện công việc',   color: '#1976d2', bg: '#e3f2fd' },
  done_worker: { key: 'done_worker', label: '✅ Đối tác báo đã hoàn thành',  color: '#388e3c', bg: '#e8f5e9' },
    ...
    return (
      <div style={{ background: '#ffebee', border: '1.5px solid #ef9a9a', borderRadius: 10, padding: '8px 12px', display: 'flex', gap: 8, alignItems: 'center' }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: '#e53935' }}>Đối tác chưa bắt đầu sau 72h. Admin đã được thông báo.</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={{ background: '#fff3e0', border: '1px solid #ffe082', borderRadius: 10, padding: '8px 12px', display: 'flex', gap: 8, alignItems: 'center' }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: '#bf360c' }}>Bấm "Bắt đầu công việc" nếu bạn đã sẵn sàng.</div>
        </div>
      </div>
    );
  }
...
export default function ServiceOrderScreen({ go, role = 'worker' }) {
  ...
  function startWork() {
    setStatus('working');
    setHours(0);
    setMsgs(m => [...m, { from: 'system', text: '🔨 Đối tác đã bắt đầu thực hiện công việc.' }]);
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', position: 'absolute', inset: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 12, background: C.g }}>
        ...
        {/* Thông tin đơn — hiện đúng 1 hoặc cả 2 trường Địa chỉ/Nền tảng tùy cờ */}
        <div style={{ background: C.w, borderRadius: 12, padding: 12, marginBottom: 10, border: '1px solid #e8def8' }}>
          ...
          {[
            ...
            { label: 'Đối tác',      val: `${workerName} (${workerSxId})` },
        ...
        {/* Nút hành động theo vai trò + trạng thái — chữ chung, chi tiết thêm theo đúng cờ */}

        {/* ĐỐI TÁC — Bắt đầu công việc */}
        {role === 'worker' && status === 'waiting' && (
          <div style={{ background: '#e3f2fd', borderRadius: 10, padding: 12, margin: '8px 0', border: '1px solid #bbdefb' }}>
            ...
            <div style={{ fontSize: 11, color: '#1976d2', marginBottom: 10 }}>
              Bấm "Bắt đầu công việc" để thông báo cho người thuê và cập nhật trạng thái đơn.
              {needsAddress && ' Đảm bảo bạn đã có mặt đúng địa chỉ.'}
              {needsContentLink && ' Đảm bảo bạn đã sẵn sàng lên nội dung theo nền tảng đã chọn.'}
            </div>
            <Btn onClick={startWork}>🔨 Bắt đầu công việc</Btn>
